...
import { MapPin, Upload, X, Plus, ArrowLeft } from "lucide-react";
import toast from "react-hot-toast";
import { venueCategories, amenitiesList } from "@/lib/utils";
import Link from "next/link";

const eventTypes = ["BANQUET_HALL", "ROOFTOP", "FARMHOUSE", "RESTAURANT", "CLUB", "CONFERENCE_ROOM", "OUTDOOR", "VILLA", "OTHER"];
            ...
            <div className="max-w-3xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="glass-card rounded-2xl p-6 space-y-4">
                        ...
                        <p className="text-white/40 text-xs">Paste image URLs (from Cloudinary, Unsplash, or your hosting). More images = more bookings!</p>
                        {imageUrls.map((url, i) => (
                    ...
                    <div className="flex gap-4">
                        ...
                        <button type="submit" disabled={loading} className="btn-primary px-8 py-3 rounded-xl flex-1 flex items-center justify-center gap-2 disabled:opacity-50">
                            {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "Submit for Approval"}
                        </button>
                    </div>
                </form>
            </div>
